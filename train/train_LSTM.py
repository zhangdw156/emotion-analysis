...
# 参数配置
class Config:
    pretrained_model_path = "./model/bert-base-uncased"  # 仍然使用BERT tokenizer
    ...
    embedding_dim = 256  # 新增LSTM参数
    hidden_dim = 128  # 新增LSTM参数
    num_layers = 2  # 新增LSTM参数
...
class LSTMModel(nn.Module):
    def __init__(self, vocab_size, embedding_dim, hidden_dim, num_layers, num_labels):
        ...
        self.lstm = nn.LSTM(embedding_dim,
                            ...
                            batch_first=True,
                            bidirectional=True)
        self.fc = nn.Linear(hidden_dim * 2, num_labels)  # 双向LSTM，所以hidden_dim要乘以2
        self.dropout = nn.Dropout(0.3)

    def forward(self, input_ids, attention_mask=None, labels=None):
        # 忽略attention_mask，因为LSTM不需要
...
def train(model, dataloader, optimizer, scheduler, device):
    ...
    for batch in progress_bar:
        ...
        labels = batch['labels'].to(device)  # LSTM不需要attention_mask
...
def main():
    ...
    # 加载tokenizer（仍然使用BERT tokenizer）
    ...
    for epoch in range(config.num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'config': config,
                'tokenizer': tokenizer,
            }, "best_model_LSTM/lstm_model.bin")
            print("Best model saved!")

    datetime_str = datetime.now().strftime("%Y%m%d_%H_%M_%S")
    dir_name: str = f'./history_model/emotion_calculate_LSTM_{datetime_str}'
    visual_dir: str = f"./history_model/visualization_LSTM_{datetime_str}"
